...
import { descriptionTextDefault } from "../../data/descriptionTextDefault";
...
function createShopLinks(buyLinks) {
  const filteredShops = stores.map(storeName => buyLinks.find(({ name }) => name === storeName));
    ...
    .map(
      ...
      <li class="stores-list__item">
        <a href="${url}" target="_blank" rel="noopener noreferrer nofollow">
          <img src="${new URL(bookShopsIcons[idx].x1, import.meta.url)}" alt="${name}" width="${bookShopsIcons[idx].width}" height="${bookShopsIcons[idx].height}">
        </a>
      </li>`
    )
    .join('');
...
function createPopupCard({ author, book_image, title, buy_links, description }, check) {
  return `
      <div class="pop-up">
        <div class="pop-up__book-info">
          ...
          <div class="pop-up__description">
            ...
            <p class="pop-up__review">${description || descriptionTextDefault}</p>
        ...
        <button class="js-popup-close close-button" type="button">
          <img src="${new URL(`../../images/icon-x-close.svg`, import.meta.url)}" width="24" height="24">
        </button>
      </div>`;
}
